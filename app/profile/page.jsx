...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchProfileAndFamilyMembers = async () => {
      try {
        const [profileRes, familyMembersRes] = await Promise.allSettled([
          fetch("/api/profile"),
        ...
        if (profileRes.status === "fulfilled") {
          ...
          setFormData({
            ...
            dateOfBirth: data.dateOfBirth ? format(new Date(data.dateOfBirth), "dd-MM-yyyy") : "",
            bloodGroup: data.bloodGroup || "",
            gender: data.gender || "",
          });
        } else {
          // Handle rejected profile fetch (e.g., network error)
          setError(profileRes.reason.message);
        }

        if (familyMembersRes.status === "fulfilled") {
          const res = familyMembersRes.value;
          if (!res.ok) {
            throw new Error(`Error: ${res.status}`);
          }
          const data = await res.json();
          ...
          // Handle rejected family members fetch
          console.error("Error fetching family members:", familyMembersRes.reason);
        }
      } catch (err) {
